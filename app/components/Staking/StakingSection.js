...
const stakingOptions = [
    { period: "30 Days", apr: "50%", bgColor: "#1C0035", textColor: "text-[#C176FF]" },
...
const StakingSection = () => {
    ...
    useEffect(() => {
        if (numericStakeAmount > 0) {
            ...
            const aprPercentage = parseFloat(selectedOption.apr.replace('%', '')) / 100;
            ...
            setUnlockDate(`${month} ${day} ${year}, ${hours}:${minutes < 10 ? '0' + minutes : minutes}`);
        } else {
                ...
                {stakingOptions.map((option, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <h3 className={`text-[20px] leading-[24px] font-medium ${isSelected ? 'text-white' : option.textColor}`}>
                                APR {option.apr}
                            </h3>
                        </div>
                    );
                ...
                <div className='w-full border border-[#440675] bg-[#1C0035] rounded-xl px-5 py-[18px] text-white space-y-[12px]' style={{ backdropFilter: "blur(30px)" }}>
                    <div className='flex items-center justify-between gap-4'>
                        ...
                        <h2 className='text-[18px] leading-[21.6px] font-normal'>
                            {numericStakeAmount > 0 ? `${formatNumberWithCommas(numericStakeAmount.toFixed(2))} VRN` : '0.00 VRN'}
                        </h2>
                    </div>
                    <div className='flex items-center justify-between gap-4'>
                        ...
                        <h2 className='text-[#C176FF] text-[18px] leading-[21.6px] font-normal'>
                            {estimatedProfit > 0 ? `${formatNumberWithCommas(estimatedProfit.toFixed(2))} VRN` : '0.00 VRN'}
                        </h2>
                    </div>
                    <div className='flex items-center justify-between gap-4'>
                        <h2 className='text-white/70 text-[18px] leading-[21.6px] font-normal underline cursor-pointer'>Total</h2>
                        <h2 className='text-[#C176FF] text-[18px] leading-[21.6px] font-normal'>
                            {totalAmount > 0 ? `${formatNumberWithCommas(totalAmount.toFixed(2))} VRN` : '0.00 VRN'}
                        </h2>
                    </div>
                </div>
                <div className='w-full border border-[#440675] bg-[#1C0035] rounded-xl px-5 py-[18px] text-white space-y-[12px]' style={{ backdropFilter: "blur(30px)" }}>
                    <div className='flex items-center justify-between gap-4'>
                        ...
                        <h2 className='text-[18px] leading-[21.6px] font-normal'>
                            {numericStakeAmount > 0 ? formatNumberWithCommas(numericStakeAmount.toFixed(2)) : '0.00'}
                        </h2>
                    </div>
                    <div className='flex items-center justify-between gap-4'>
